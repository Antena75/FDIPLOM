  ...
  const formHandler = async (e: any) => {
    try {
      ...
      librariesAPI.addLibrary(formData)
        .then(result => {
          iziToast.success({
            message: `Библиотека ${result.data.title} успешно добавлена`,
            position: 'bottomCenter',
          });
  ...
  return (
    <Form className="mb-3" onSubmit={formHandler}>
      <Form.Group className="mb-3">
        ...
        <Form.Control type="text" className="mb-3" placeholder="Введите название библиотеки" onChange={(e) => setName(e.target.value)} required />
      </Form.Group>

      <Form.Group className="mb-3">
        <Form.Label>Адрес библиотеки (5 - 100 символов)</Form.Label>
        <Form.Control type="text" className="mb-3" placeholder="Введите адрес библиотеки" onChange={(e) => setAddress(e.target.value)} required />
      </Form.Group>

      <Form.Group className="mb-3">
        <Form.Label>Описание (необязательно, не больше 200 символов)</Form.Label>
        <Form.Control as="textarea" rows={3} className="mb-3" maxLength={200} placeholder="Введите описание библиотеки" onChange={(e) => setDescription(e.target.value)} />
      </Form.Group>
...
export default LibrariesAddForm