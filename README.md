Запуск приложения:
1. Запуск MongoDB и MongoDB-Express в Docker в папке 'app/backend': docker compose -f docker-compose.DB.yml up
...
4. Docker-контейнер собирается в папке 'app': docker compose -f docker-compose.dev.yml up
5. Добавил ещё файл: 'docker-compose.yml'
6. Примечание:
   - Картинки библиотек и книг загружаются из backend в папку 'frontend/public/libraries', потом frontend их забирает;
...
- Интерфейс может быть реализован по вашему усмотрению. Время, отводимое на дипломную работу, не предполагает существенных усилий по оформлению приложения. Ознакомьтесь с нашим предложением по интерфейсу по ссылке:
  [Визуальная часть проекта](https://www.figma.com/)
